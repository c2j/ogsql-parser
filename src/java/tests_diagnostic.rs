...
fn diag_a1_select_box_not_sql() {
    ...
    assert_eq!(result.extractions.len(), 0, "SELECT_BOX is not SQL, got: {:?}", result.extractions);
}
...
fn diag_a4_english_text_not_sql() {
    ...
    assert_eq!(
        result.extractions.len(),
        0,
        "English prose with SQL keyword should not be extracted, got: {:?}",
        result.extractions
    );
}
...
fn diag_b3_field_stringbuilder_in_method() {
    ...
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(
        result.extractions.len() >= 1,
        "field-level StringBuilder should be tracked; \
        needs visit_field_declaration to handle StringBuilder type, got count: {}",
        result.extractions.len()
    );
    let combined = result.extractions.iter().map(|e| e.sql.as_str()).collect::<String>();
    assert!(
        combined.contains("WHERE id ="),
        "field SB append in method should be tracked, got: {:?}",
        result.extractions
    );
}
...
fn diag_c1_append_insert_append_chain() {
    ...
    assert!(
        ...
        needs handle_sb_append to recurse into insert/delete, got: {}",
        sql
    );
...
fn diag_c2_stringbuilder_replace() {
    ...
    let sql = &result.extractions[0].sql;
    assert!(
        sql.contains("users") && !sql.contains("old_table"),
        "StringBuilder.replace should be applied; \
        needs handle_sb_replace implementation, got: {}",
        sql
    );
}
...
fn diag_c3_delete_then_append_chain() {
    ...
    assert_eq!(result.extractions.len(), 1);
    let sql = &result.extractions[0].sql;
    assert!(!sql.contains("obsolete"), "delete should remove 'obsolete', got: {}", sql);
    assert!(
        sql.contains("WHERE id = 1"),
        "append after delete in chain should work; \
        needs handle_sb_delete to propagate chain to append, got: {}",
        sql
    );
}
...
fn diag_d1_constant_propagation_local() {
    ...
    let sql = &result.extractions[0].sql;
    assert!(sql.contains("SELECT * FROM users"), "final String constant should be inlined into SQL, got: {}", sql);
}
...
fn diag_d2_constant_propagation_static_field() {
    ...
    assert_eq!(result.extractions.len(), 1);
    let sql = &result.extractions[0].sql;
    assert!(sql.contains("SELECT * FROM users"), "static final field constant should be inlined, got: {}", sql);
}
...
fn diag_d3_getter_method_return() {
    ...
    let sql = &result.extractions[0].sql;
    assert!(sql.contains("SELECT * FROM"), "should extract SQL containing method call, got: {}", sql);
    assert!(
        !sql.contains("getTableName()") && !sql.contains("____"),
        "placeholder for getter call should be clean, got: {}",
        sql
    );
}
...
fn diag_d4_field_access_type() {
    ...
    let sql = &result.extractions[0].sql;
    assert!(
        !sql.contains("user.name") && !sql.contains("user_name"),
        "field access user.name should produce a clean placeholder for the field part, got: {}",
        sql
    );
}
...
fn diag_d5_generic_list_get() {
    ...
    let sql = &result.extractions[0].sql;
    assert!(
        !sql.contains("names.get(0)") && !sql.contains("names_get_0___"),
        "names.get(0) should produce a clean placeholder, got: {}",
        sql
    );
}
...
fn diag_f2_sql_built_inside_lambda() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert_eq!(result.extractions.len(), 1, "SQL built inside lambda body should be extracted");
    assert!(result.extractions[0].sql.contains("SELECT COUNT(*) FROM"), "got: {}", result.extractions[0].sql);
}
...
fn diag_f3_sql_inside_anonymous_class() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert_eq!(result.extractions.len(), 1, "SQL inside anonymous class should be extracted");
...
fn diag_g1_scoped_prepared_statement() {
    ...
    assert_eq!(result.extractions.len(), 1);
    assert!(
        result.extractions[0].sql.contains("__JAVA_VAR_String_name__"),
        "scoped PS type should still resolve setString backfill, got: {}",
        result.extractions[0].sql
    );
    assert!(
        !result.extractions[0].sql.contains("__JAVA_VAR_JDBC_PARAM_1__"),
        "param should be backfilled, not left as JDBC_PARAM, got: {}",
        result.extractions[0].sql
    );
}
...
fn diag_g2_scoped_ps_as_method_param() {
    ...
    let sql = &result.extractions[0].sql;
    assert!(
        sql.contains("__JAVA_VAR_String_name__"),
        "scoped_type_identifier in method param should match 'PreparedStatement' for behavior recording; \
        needs extract_type_name to handle scoped_type_identifier, got: {}",
        sql
    );
    assert!(!sql.contains("DYNAMIC"), "should not fall back to DYNAMIC when setter pattern is available, got: {}", sql);
}
...
fn diag_h1_ternary_operator_sql() {
    ...
    assert!(
        ...
        needs handling for ternary_expression node, got count: {}",
        result.extractions.len()
    );
}
...
fn diag_h2_switch_statement_sql() {
    ...
    assert!(
        all_sql.contains("SELECT") || all_sql.contains("DELETE"),
        "should contain SELECT or DELETE SQL, got: {:?}",
        result.extractions
    );
}
...
fn diag_h3_nested_class_field_sql() {
    ...
    assert!(
        ...
        got count: {}, extractions: {:?}",
        result.extractions.len(),
        result.extractions
    );
    let all_sql: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
    assert!(all_sql.contains("inner_table"), "should contain inner_table, got: {:?}", result.extractions);
    assert!(
        all_sql.contains("WHERE id = 1"),
        "method body concat should extend field SQL, got: {:?}",
        result.extractions
    );
}
...
fn diag_h5_multiple_classes_same_file() {
    ...
    assert!(
        ...
        needs method_behaviors shared across class declarations in same file, got: {}",
        sql
    );
    assert!(
        !sql.contains("DYNAMIC"),
        "should not fall back to DYNAMIC when literal setter pattern exists, got: {}",
        sql
    );
}
...
fn diag_i1_two_classes_cross_method() {
    ...
    let sql = &result.extractions[0].sql;
    assert!(
        sql.contains("__JAVA_VAR_String_orderId__"),
        "cross-class PS should resolve typed params; \
        needs method_behaviors shared across class declarations, got: {}",
        sql
    );
    assert!(sql.contains("__JAVA_VAR_String_product__"), "product param should also be typed, got: {}", sql);
    assert!(!sql.contains("DYNAMIC"), "should not fall back to DYNAMIC, got: {}", sql);
}
...
fn diag_j1_sb_with_jdbc_setters_and_cross_method() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "should extract at least the SB-assembled SQL, got count: {}",
        result.extractions.len()
    );
    let all_sql: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
    assert!(all_sql.contains("WHERE dept"), "SB appends should appear in final SQL, got: {:?}", result.extractions);
    assert!(
        all_sql.contains("__JAVA_VAR_String_dept__"),
        "cross-method JDBC setter should backfill typed params, got: {:?}",
        result.extractions
    );
}
...
fn diag_j2_annotation_plus_method_call_plus_constant() {
    ...
    assert!(
        result.extractions.len() >= 3,
        "should extract from annotation + constant + method call, got: {}",
        result.extractions.len()
    );
...
fn diag_j5_text_block_with_params() {
    ...
    assert!(
        result.extractions[0].sql.contains("__JAVA_VAR_int_age__"),
        "age param, got: {}",
        result.extractions[0].sql
    );
}
...
fn diag_j6_string_concat_with_method_call_value() {
    ...
    assert_eq!(result.extractions.len(), 1);
    assert!(
        result.extractions[0].sql.contains("SELECT * FROM"),
        "SQL with method call should be extracted, got: {}",
        result.extractions[0].sql
    );
}
...
fn diag_j7_sb_concat_with_ps_cross_method_complex() {
    ...
    let all_sql: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
    assert!(
        all_sql.contains("WHERE u.name LIKE"),
        "SB appends with JDBC params should be assembled, got: {:?}",
        result.extractions
    );
    assert!(
        all_sql.contains("__JAVA_VAR_String_name__"),
        "cross-class method should resolve name param, got: {:?}",
        result.extractions
    );
    assert!(
        all_sql.contains("__JAVA_VAR_String_dept__"),
        "cross-class method should resolve dept param, got: {:?}",
        result.extractions
    );
    assert!(
        all_sql.contains("__JAVA_VAR_int_minAge__"),
        "cross-class method should resolve minAge param, got: {:?}",
        result.extractions
    );
}
...
fn diag_j8_overloaded_method_name() {
    ...
    assert!(result.extractions.len() >= 2, "should extract both INSERT statements, got: {}", result.extractions.len());
    ...
    assert!(
        user.sql.contains("__JAVA_VAR_String_name__"),
        "user insert: overloaded bind(String) should resolve, \
        needs methodBehaviors keyed by signature not just name, got: {}",
        user.sql
    );
    let order = order_sql.unwrap();
    assert!(
        order.sql.contains("__JAVA_VAR_String_orderId__"),
        "order insert: overloaded bind(String,int) should resolve, got: {}",
        order.sql
    );
    assert!(order.sql.contains("__JAVA_VAR_int_amount__"), "order insert: amount param, got: {}", order.sql);
}
...
fn diag_k1_if_else_both_sql() {
    ...
    assert!(
        result.extractions.len() >= 2,
        "both if/else branches should produce extractions, got: {}",
        result.extractions.len()
    );
...
fn diag_k2_if_else_chain() {
    ...
    assert!(
        result.extractions.len() >= 3,
        "all 3 branches should produce extractions, got: {}",
        result.extractions.len()
    );
}
...
fn diag_k3_nested_if_sql() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 1, "nested if should extract SQL, got: {}", result.extractions.len());
    let all: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
...
fn diag_k4_if_with_concat_var() {
    ...
    assert!(
        result.extractions.len() >= 2,
        "both branches with concat should extract, got: {}",
        result.extractions.len()
    );
    let all: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
...
fn diag_k5_if_else_sb_append() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "SB inside if/else should produce extraction, got: {}",
        result.extractions.len()
    );
    let sql = &result.extractions[0].sql;
...
fn diag_k6_only_if_no_else_sql() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 1, "if-only branch with += should extract, got: {}", result.extractions.len());
}
...
fn diag_l1_sql_inside_for_body() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "SQL inside for body should be extracted, got: {}",
        result.extractions.len()
    );
...
fn diag_l2_sb_append_in_for() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "SB assembled in for loop should extract, got: {}",
        result.extractions.len()
    );
    let sql = &result.extractions[0].sql;
...
fn diag_l3_for_each_sql() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "SQL inside for-each should be extracted, got: {}",
        result.extractions.len()
    );
    let all: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
...
fn diag_l5_nested_for_sb_in() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "nested for building IN clause should extract, got: {}",
        result.extractions.len()
    );
    let sql = &result.extractions[0].sql;
...
fn diag_l6_while_loop_sql() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 1, "SQL inside while should be extracted, got: {}", result.extractions.len());
}
...
fn diag_l7_do_while_sql() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "SQL inside do-while should be extracted, got: {}",
        result.extractions.len()
    );
...
fn diag_m1_switch_multiple_cases() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 4, "all 4 switch branches should extract, got: {}", result.extractions.len());
    let all: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
...
fn diag_m2_switch_fallthrough() {
    ...
    assert!(
        result.extractions.len() >= 2,
        "switch fallthrough cases should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_m3_switch_with_sb() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 1, "SB inside switch should extract, got: {}", result.extractions.len());
}
...
fn diag_m4_switch_expression_java14() {
    ...
    assert!(
        ...
        currently fails because tree-sitter switch_expression node is not traversed, got: {}",
        result.extractions.len()
    );
}
...
fn diag_m5_switch_with_string_concat() {
    ...
    assert!(
        result.extractions.len() >= 2,
        "switch with concatenated SQL should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_n1_try_catch_finally() {
    ...
    assert!(
        result.extractions.len() >= 3,
        "try + catch + finally should each extract, got: {}",
        result.extractions.len()
    );
    let all: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
...
fn diag_n2_try_with_resources_sql() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "SQL in try-with-resources should extract, got: {}",
        result.extractions.len()
    );
    let sql = &result.extractions[0].sql;
...
fn diag_n3_nested_try_catch() {
    ...
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(
        result.extractions.len() >= 4,
        "all 4 nested try/catch SQLs should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_n4_multi_catch() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 2, "try + multi-catch should extract, got: {}", result.extractions.len());
}
...
fn diag_o1_if_inside_for_sb() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 1, "if inside for with SB should extract, got: {}", result.extractions.len());
}
...
fn diag_o2_sb_conditional_where() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "conditional WHERE building should extract base SQL, got: {}",
        result.extractions.len()
    );
    let sql = &result.extractions[0].sql;
...
fn diag_o3_switch_in_for() {
    ...
    assert!(
        result.extractions.len() >= 3,
        "switch inside for should extract each branch, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o4_for_inside_try_inside_if() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 2, "nested if > try > for should extract, got: {}", result.extractions.len());
}
...
fn diag_o5_labeled_break_in_for() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 1, "SQL inside labeled for should extract, got: {}", result.extractions.len());
}
...
fn diag_o6_synchronized_block_sql() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "SQL inside synchronized block should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o10_return_sql_from_if() {
    ...
    assert!(
        ...
        inside return is not visited as a top-level declaration/assignment, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o11_break_in_switch_with_sql() {
    ...
    assert!(
        result.extractions.len() >= 2,
        "switch with break and PS should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o12_complex_daopattern() {
    ...
    assert!(result.extractions.len() >= 1, "complex DAO pattern should extract SQL, got: {}", result.extractions.len());
    let all_sql: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
...
fn diag_p1_cross_file_ps_binding() {
    ...
    let dao = &results[1];
    assert!(dao.extractions.len() >= 1, "should extract INSERT from UserDao, got: {}", dao.extractions.len());
    let sql = &dao.extractions[0].sql;
    assert!(
        sql.contains("__JAVA_VAR_String_name__"),
        "cross-file PS binding should resolve setter; \
        needs method_behaviors shared across files, got: {}",
        sql
    );
    assert!(!sql.contains("DYNAMIC"), "should not fall back to DYNAMIC, got: {}", sql);
}
...
fn diag_p2_cross_file_multi_param_binding() {
    ...
    let sql = &dao.extractions[0].sql;
    assert!(sql.contains("__JAVA_VAR_String_orderId__"), "orderId from cross-file helper, got: {}", sql);
    assert!(sql.contains("__JAVA_VAR_String_product__"), "product from cross-file helper, got: {}", sql);
    assert!(sql.contains("__JAVA_VAR_int_qty__"), "qty from cross-file helper, got: {}", sql);
}
...
fn diag_p3_cross_file_overloaded_methods() {
    ...
    let dao = &results[1];
    assert!(dao.extractions.len() >= 2, "should extract both INSERTs, got: {}", dao.extractions.len());
    ...
    assert!(
        user_sql.unwrap().sql.contains("__JAVA_VAR_String_name__"),
        "user insert: cross-file overloaded bind(String), got: {}",
        user_sql.unwrap().sql
    );
    assert!(
        order_sql.unwrap().sql.contains("__JAVA_VAR_String_orderId__"),
        "order insert: cross-file overloaded bind(String,int), got: {}",
        order_sql.unwrap().sql
    );
}
...
fn diag_p4_cross_file_sb_with_constant() {
    ...
    let dao = &results[1];
    assert!(dao.extractions.len() >= 1, "cross-file constant in concat should extract, got: {}", dao.extractions.len());
    let sql = &dao.extractions[0].sql;
    assert!(sql.contains("SELECT id, name FROM users"), "constant value should be inlined, got: {}", sql);
    assert!(sql.contains("WHERE active = 1"), "local concat should be preserved, got: {}", sql);
}
...
fn diag_p5_cross_file_sb_init_with_constant() {
    ...
    let dao = &results[1];
    assert!(
        dao.extractions.len() >= 1,
        "SB initialized with cross-file constant should extract, got: {}",
        dao.extractions.len()
    );
    let sql = &dao.extractions[0].sql;
...
fn diag_p6_cross_file_annotation_plus_constant() {
    ...
    "#;
    let results = extract_sql_from_java_files(
        &[("SqlConstants.java", constants_java), ("UserRepo.java", repo_java), ("UserDao.java", dao_java)],
        &JavaExtractConfig::default(),
    );
...
fn diag_p7_cross_file_ps_through_service_layer() {
    ...
    assert!(sql.contains("__JAVA_VAR_String_name__"), "cross-file binding through service layer, got: {}", sql);
    assert!(sql.contains("__JAVA_VAR_String_email__"), "email param, got: {}", sql);
}
...
fn diag_p8_cross_file_three_layer_chain() {
    ...
    "#;
    let results = extract_sql_from_java_files(
        &[("ParamBinder.java", binder_java), ("UserService.java", service_java), ("UserDao.java", dao_java)],
        &JavaExtractConfig::default(),
    );
    let dao = &results[2];
    assert!(dao.extractions.len() >= 1);
    let sql = &dao.extractions[0].sql;
    assert!(sql.contains("__JAVA_VAR_String_name__"), "3-layer cross-file binding: name, got: {}", sql);
    assert!(sql.contains("__JAVA_VAR_int_age__"), "3-layer cross-file binding: age, got: {}", sql);
}
...
fn diag_p9_cross_file_dynamic_loop_setter() {
    ...
    let sql = &dao.extractions[0].sql;
    assert!(
        sql.contains("__JAVA_VAR_String_DYNAMIC_"),
        "cross-file dynamic loop setter should produce DYNAMIC placeholders, got: {}",
        sql
    );
}
...
fn diag_p10_cross_file_reverse_order() {
    ...
    assert!(
        dao.extractions.len() >= 1,
        "Dao processed BEFORE Binder: method_behaviors from Binder not yet available, got: {}",
        dao.extractions.len()
    );
    let sql = &dao.extractions[0].sql;
...
fn diag_p11_cross_file_sb_with_ps_and_constant() {
    ...
    "#;
    let results = extract_sql_from_java_files(
        &[("Sql.java", constants_java), ("ReportHelper.java", helper_java), ("ReportDao.java", dao_java)],
        &JavaExtractConfig::default(),
    );
    let dao = &results[2];
    assert!(dao.extractions.len() >= 1);
    ...
    assert!(sql.contains("__JAVA_VAR_String_dept__"), "cross-file binding, got: {}", sql);
}
...
fn diag_p12_cross_file_inheritance_field_sql() {
    ...
    assert!(
        dao.extractions.len() >= 1,
        "child class using inherited method should extract SQL, got: {}",
        dao.extractions.len()
    );
}
...
fn diag_q1_cross_file_nonexistent_method() {
    ...
    "#;
    let results = extract_sql_from_java_files(&[("Dao.java", dao_java)], &JavaExtractConfig::default());
    let dao = &results[0];
    assert!(
        dao.extractions.len() >= 1,
        "should still extract SQL even if helper method not found, got: {}",
        dao.extractions.len()
    );
    let sql = &dao.extractions[0].sql;
    assert!(sql.contains("INSERT INTO users"), "base SQL, got: {}", sql);
...
fn diag_q2_cross_file_empty_helper() {
    ...
    let dao = &results[1];
    assert!(dao.extractions.len() >= 1, "unrelated helper file should not interfere, got: {}", dao.extractions.len());
}
...
fn diag_q3_cross_file_constant_only_used_in_sb() {
    ...
    assert!(
        dao.extractions.len() >= 1,
        "cross-file constant as SB append arg should inline, got: {}",
        dao.extractions.len()
    );
    let sql = &dao.extractions[0].sql;
    assert!(sql.contains("SELECT * FROM"), "base, got: {}", sql);
    assert!(sql.contains("WHERE active = 1"), "append, got: {}", sql);
}
...
fn diag_q4_cross_file_same_class_name_different_files() {
    ...
    "#;
    let results = extract_sql_from_java_files(
        &[("Binder1.java", helper1_java), ("Binder2.java", helper2_java), ("Dao.java", dao_java)],
        &JavaExtractConfig::default(),
    );
    let dao = &results[2];
    assert!(dao.extractions.len() >= 1);
...
fn diag_q5_cross_file_concat_with_concat_value_in_setter() {
    ...
    assert!(dao.extractions.len() >= 1);
    let sql = &dao.extractions[0].sql;
    assert!(sql.contains("__JAVA_VAR_String_name__"), "concat value in cross-file setter should resolve, got: {}", sql);
    assert!(sql.contains("__JAVA_VAR_String_dept__"), "simple value in cross-file setter, got: {}", sql);
}
...
fn diag_m6_switch_expression_assigned_to_var() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "switch expression assigned to var should extract SQL cases, got: {}",
        result.extractions.len()
    );
}
...
fn diag_m7_switch_expression_with_concat() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "switch expression with concat should extract SQL, got: {}",
        result.extractions.len()
    );
}
...
fn diag_m8_switch_expression_in_method_arg() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "switch expression as method argument should extract SQL, got: {}",
        result.extractions.len()
    );
}
...
fn diag_m9_switch_expression_only_non_sql() {
    ...
    assert!(
        ...
        "switch expression with no SQL should not extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_m10_switch_expression_with_placeholders() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "switch expression with JDBC placeholders should extract, got: {}",
        result.extractions.len()
    );
    let all_sql: String = result.extractions.iter().map(|e| e.sql.as_str()).collect();
    assert!(all_sql.contains("WHERE id"), "placeholder-containing SQL should appear, got: {:?}", result.extractions);
}
...
fn diag_m11_switch_expression_nested_in_sb_append() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "switch expression inside SB append should produce assembled SQL, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o13_return_sql_from_if() {
    ...
    assert!(
        result.extractions.len() >= 2,
        "return SQL from both if/else branches should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o14_return_sql_from_switch() {
    ...
    assert!(
        result.extractions.len() >= 2,
        "return SQL from switch cases should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o15_return_sql_from_ternary() {
    ...
    "#;
    let result = extract_sql_from_java(java, "Dao.java", &JavaExtractConfig::default());
    assert!(result.extractions.len() >= 1, "return SQL from ternary should extract, got: {}", result.extractions.len());
}
...
fn diag_o16_return_sql_only_non_sql() {
    ...
    assert!(
        result.extractions.is_empty(),
        "return of non-SQL strings should not extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o17_return_sql_with_concat() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "return of concatenated SQL should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o18_return_sql_deep_nesting() {
    ...
    assert!(
        result.extractions.len() >= 2,
        "return SQL from deeply nested if should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o19_return_sql_used_by_caller() {
    ...
    assert!(
        result.extractions.len() >= 1,
        "SQL returned from method and used by caller should extract, got: {}",
        result.extractions.len()
    );
}
...
fn diag_o20_return_sql_in_try_catch() {
    ...
    assert!(
        result.extractions.len() >= 2,
        "return SQL inside try/catch should extract, got: {}",
        result.extractions.len()
    );
}
